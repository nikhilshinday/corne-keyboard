...
### Regenerate locally (uv)

```bash
./scripts/draw-keymap.sh
```

Requires `uv` in your PATH. The script uses `uvx` to run keymap-drawer and writes
`keymap-drawer/eyelash_corne.yaml` and `keymap-drawer/eyelash_corne.svg`.
